...
export default function createJobObservable(driver, { ignorePageError = false } = {}) {
  return new Observable(observer => {
    ...
    (async function() {
      for (;;) {
        const job = await Promise.race([driver.executeScript(() => window.__test__.jobs.acquire()), abortPromise]);
        ...
        if (job === ABORT_SYMBOL) {
          ...
          await deferred.promise.then(
            (result = null) =>
              driver.executeScript((id, result) => window.__test__.jobs.resolve(id, result), job.id, result),
            async error => {
              await driver.executeScript(
                (id, stack) => window.__test__.jobs.reject(id, new Error(stack)),
                job.id,
                error.stack
              );
